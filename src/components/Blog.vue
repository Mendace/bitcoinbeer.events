<template>
	<section class="text-white py-12">
	  <div class="container p-6 mx-auto space-y-8">
		<div class="space-y-2 text-center">
		  ...
		  <p class="font-serif text-lg text-gray-400">
			Ultimi articoli pubblicati nel blog
		  </p>
		</div>
		<!-- Griglia articoli: mostra solo i primi 3 -->
		...
		<!-- Pulsante "Vedi tutti" se ci sono più di 3 articoli -->
		<div class="text-center" v-if="articles.length > 3">
		  <!-- Se usi il router di Vue, puoi fare un router-link -->
		  <!-- <router-link
			to="/BlogMain"
			class="inline-block px-6 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-md shadow-md transition"
		  >
			Vedi tutti
		  </router-link> -->
  
		  <!-- Oppure un semplice link se preferisci -->
		  <a
			href="/BlogMain"
			class="inline-block px-6 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-md shadow-md transition"
		  >
  ...
  export default {
	...
	computed: {
	  // Mostriamo solo i primi 3 articoli
	  displayedArticles() {
	...
	mounted() {
	  ...
	  fetch('https://api.bitcoinbeer.events/api/articles_api.php')
		...
		.then((data) => {
		  if (data.success) {
			// Assegna tutti gli articoli, ma "displayedArticles" ne mostrerà solo 3
